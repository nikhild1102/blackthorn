...
validateExtend.configureI18n();
validateExtend.validateDateFormat();
...
class ServiceBase {
  ...
  addError (attribute, errorMessage, responseStatusCode = null) {
    ...
    const errors = this._errors = this._errors[this.constructor.name] || {}
    if (!errors[attribute]) return _.extend(errors, { ["message"]: `${errorMessage}` })
    errors[attribute] = errors[attribute] instanceof Array ? errors[attribute] : [errors[attribute]]
    errors[attribute].push(`${errorMessage}`)
  }
  ...
  async validateServiceInputs () {
    ...
    if (_.size(errors)) {
      _.extend(this.errors, {"errors":errors})
